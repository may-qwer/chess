...
Board::Board() {
    board_mtx = new Figure**[SIZE_OF_BOARD];
    for (int i = 0; i < SIZE_OF_BOARD; i++) {
        board_mtx[i] = new Figure*[SIZE_OF_BOARD];
    }
    set_empty_to_cell();
...
void Board::show() {
    for (int i = 0; i < SIZE_OF_BOARD; i++) {
        ...
        for (int j = 0; j < SIZE_OF_BOARD; j++) {
            if ((i + j)%2 == 0) {
                cout << BACKGROUND_WHITE << " " << board_mtx[i][j].show() << " " << RESET;
            } else {
                cout << BACKGROUND_BLACK << " " << board_mtx[i][j].show() << " " << RESET;
            }
        }
        cout << endl;
    }
...
void Board::set_start_pos(const char *pos_str) { 
    ...
    for (int i = 0; i < COUNT_OF_FIGURES_SIMBOLS; i += 2) {
        ...
        if (pos_str[i] == '-') {
            continue;
        } else if (i <= arr_of_indexes_of_figures[0]) {
            board_mtx[num_pos][letter_pos] = Pawn('w', );
        }
    }
